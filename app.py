...
def __trace_recursive_enter__(func_name, locals_dict):
    frame = inspect.currentframe().f_back
    call_info = {
        'function': func_name,
    ...
    tracker = getattr(__visualize__, 'tracker', None)
    time.sleep(tracker.animation_speed if tracker else 0.5)
def __trace_recursive_exit__(func_name):
    frame = inspect.currentframe().f_back
    call_info = {
        'function': func_name,
        'line': frame.f_lineno,
...
def __custom_print__(*args, **kwargs):
    ...
    print(*args, **kwargs)  
def __track_loop__(loop_id, loop_type, is_enter):
...
def handle_run_code(data):
    ...
    try:
        ...
        namespace = {
            ...
            '__trace_recursive_enter__': __trace_recursive_enter__,
            '__trace_recursive_exit__': __trace_recursive_exit__,
            '__track_loop__': __track_loop__,
            'time': time,
            'deque': deque
        }
...
def bfs(graph, start_node):
    ...
    return visited
def dfs(graph, node, visited=None, path=None, parent=None):
    if visited is None:
        visited = set()
    if path is None:
        path = []  
    ...
    for neighbor in graph.get(node, []):
        if neighbor not in visited:
            dfs(graph, neighbor, visited, path, node)
    path.pop()  
    return visited
def binary_tree_traversal(root, order='inorder'):
    def inorder(node):
        if node:
            inorder(node.left)
            __visualize__({'current': node, 'tree': root})
            inorder(node.right)
    def preorder(node):
        if node:
            __visualize__({'current': node, 'tree': root})
